...
`0.3.0-alpha.2-dev`). GoReleaser replaces it through `-ldflags -X` with the exact
tag version, so a `v0.3.0-alpha.2` artifact reports `wipeme 0.3.0-alpha.2`.
...
as `v0.3.0-alpha.2`. Publish the generated `dist/homebrew/Casks/wipeme.rb` to
...
6. Create and push an annotated tag, for example:
   ...
   ```sh
   git tag -a v0.3.0-alpha.2 -m "wipeme v0.3.0-alpha.2"
