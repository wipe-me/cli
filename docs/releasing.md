...
`0.2.2-alpha.1-dev`). GoReleaser replaces it through `-ldflags -X` with the exact
tag version, so a `v0.2.2-alpha.1` artifact reports `wipeme 0.2.2-alpha.1`.
...
as `v0.2.2-alpha.1`. Publish the generated `dist/homebrew/Casks/wipeme.rb` to
...
6. Create and push an annotated tag, for example:
   ...
   ```sh
   git tag -a v0.2.2-alpha.1 -m "wipeme v0.2.2-alpha.1"
   git push origin v0.2.2-alpha.1
   ```
